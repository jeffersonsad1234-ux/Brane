          ...
          <Tabs defaultValue="login">
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <div>
                  ...
                  <Input type="password" value={loginData.password} onChange={e => setLoginData({...loginData, password: e.target.value})}
                    required className="bg-[#111] border-[#2A2A2A] text-white" data-testid="login-password" />
                </div>
            ...
            <TabsContent value="register">
              <form onSubmit={handleRegister} className="space-y-4">
                <div>
                  <Label className="text-[#CCC]">Nome</Label>
                  <Input value={regData.name} onChange={e => setRegData({...regData, name: e.target.value})}
                    required className="bg-[#111] border-[#2A2A2A] text-white" data-testid="register-name" />
                </div>
                <div>
                  <Label className="text-[#CCC]">Email</Label>
                  <Input type="email" value={regData.email} onChange={e => setRegData({...regData, email: e.target.value})}
                    required className="bg-[#111] border-[#2A2A2A] text-white" data-testid="register-email" />
                </div>
                <div>
                  <Label className="text-[#CCC]">Senha</Label>
                  <Input type="password" value={regData.password} onChange={e => setRegData({...regData, password: e.target.value})}
                    required minLength={6} className="bg-[#111] border-[#2A2A2A] text-white" data-testid="register-password" />
                </div>
