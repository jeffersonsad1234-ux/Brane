...
export const FMT = (s) => { const m = Math.floor(s / 60); const sec = Math.floor(s % 60); const ms = Math.floor((s % 1) * 100); return `${String(m).padStart(2, "0")}:${String(sec).padStart(2, "0")}.${String(ms).padStart(2, "0")}`; };
...
export const COLORS = {
  ...
  track: {
    video: { bar: "#2563eb", bg: "rgba(37,99,235,0.12)", bd: "rgba(37,99,235,0.25)" },
    audio: { bar: "#22c55e", bg: "rgba(34,197,94,0.12)", bd: "rgba(34,197,94,0.25)" },
...
export const S = ({ d, sz = 14, style }) => <svg style={{ width: sz, height: sz, flexShrink: 0, ...style }} viewBox="0 0 24 24" fill="currentColor"><path d={d} /></svg>;
...
export const I = {
  ...
  tiktok: "M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.87 2.45c-1.59 0-2.88-1.28-2.88-2.86s1.29-2.86 2.88-2.86c.23 0 .45.03.66.08V8.34a6.32 6.32 0 0 0-.66-.04c-3.49 0-6.33 2.82-6.33 6.3S5.5 20.9 9 20.9s6.33-2.82 6.33-6.3V10.4A8.15 8.15 0 0 0 19.59 6.69z",
...
export const Rng = ({ min, max, val, onChange, cls = "", step = 1 }) => <input type="range" min={min} max={max} step={step} value={val} onChange={onChange} className={`w-full h-[3px] accent-emerald-500 bg-white/[0.06] rounded-full appearance-none cursor-pointer ${cls}`} />;

export const Tp = ({ text, ch }) => <div className="group relative inline-flex">{ch}<div className="absolute -top-7 left-1/2 -translate-x-1/2 px-2 py-0.5 rounded bg-[#1a1a1a] border border-white/10 text-[9px] text-white/50 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 shadow-lg">{text}</div></div>;
...
export const SIDEBAR_MAP = {
  ...
  effects: I.efx, luts: I.lut, color: I.adj, motion: I.motion, ai: I.ai,
  assets: I.lay, templates: I.doc, captions: I.captions, brand: I.brand, memory: I.memory,
};
...
export const BRAND_ASSETS = [
  { id: "ba1", name: "Logo Primary.png", type: "image", thumb: "🏢" }, { id: "ba2", name: "Logo Dark.png", type: "image", thumb: "🌙" },
  { id: "ba3", name: "Colors.ase", type: "palette", thumb: "🎨" }, { id: "ba4", name: "Font Pack.zip", type: "font", thumb: "🔤" },
  { id: "ba5", name: "Watermark.png", type: "image", thumb: "®️" }, { id: "ba6", name: "Pattern.png", type: "image", thumb: "🔷" },
];
...
export const TEMPLATES = [
  ...
  { id: "tp11", name: "TikTok Viral", dur: 12, thumb: "🔥" }, { id: "tp12", name: "Gamer", dur: 20, thumb: "🎮" },
];
...
export function badge(type) {
  return ({ video: { bg: "#1e40af", l: "V" }, audio: { bg: "#166534", l: "A" }, text: { bg: "#92400e", l: "T" }, sticker: { bg: "#6b21a8", l: "S" }, overlay: { bg: "#831843", l: "O" } })[type] || { bg: "#333", l: "?" };
}
