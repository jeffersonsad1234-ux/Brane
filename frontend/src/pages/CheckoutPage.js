      ...
      <div className="min-h-screen carbon-bg py-8">
        <div className="max-w-2xl mx-auto px-4">
          <div className="dark-card rounded-xl p-8 text-center animate-fadeIn">
            ...
            <p className="text-[#888] mb-2">Pedido #{orderComplete.order_id?.slice(0, 16)}</p>
            <p className="text-2xl font-bold text-[#B38B36] mb-6">R$ {orderComplete.total?.toFixed(2)}</p>

            <div className="bg-[#111] rounded-xl p-6 text-left mb-6 border border-yellow-500/30">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#888]">Copie a chave e pague pelo seu banco</p>
                    </div>
                  </div>
                  <div className="bg-[#0A0A0A] rounded-lg p-4 border border-[#2A2A2A]">
                    <p className="text-xs text-[#888] mb-1">Chave PIX ({pi.pix_key_type === 'cpf' ? 'CPF/CNPJ' : pi.pix_key_type === 'email' ? 'E-mail' : pi.pix_key_type === 'phone' ? 'Telefone' : 'Aleatoria'})</p>
                    <div className="flex items-center justify-between">
                      <p className="text-white font-mono text-lg break-all">{pi.pix_key}</p>
                      <button onClick={() => copyToClipboard(pi.pix_key)} className="text-[#B38B36] hover:text-[#D4A842] p-2 shrink-0">
                        <Copy className="w-5 h-5" />
                      </button>
                    </div>
                  </div>
                  <p className="text-sm text-[#888]">Valor: <span className="text-white font-bold">R$ {orderComplete.total?.toFixed(2)}</span></p>
                </div>
              )}
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#888]">Transfira para a conta abaixo</p>
                    </div>
                  </div>
                  <div className="bg-[#0A0A0A] rounded-lg p-4 border border-[#2A2A2A] space-y-3">
                    {[
                      ['Banco', pi.bank_name],
                      ...
                      <div key={label} className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => copyToClipboard(val)} className="text-[#B38B36] hover:text-[#D4A842]">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#888]">Envie para o email abaixo</p>
                    </div>
                  </div>
                  <div className="bg-[#0A0A0A] rounded-lg p-4 border border-[#2A2A2A]">
                    <div className="flex items-center justify-between">
                      <p className="text-white font-medium">{pi.paypal_email}</p>
                      <button onClick={() => copyToClipboard(pi.paypal_email)} className="text-[#B38B36] hover:text-[#D4A842] p-2">
                        <Copy className="w-5 h-5" />
                      </button>
                    </div>
                  </div>
            ...
            <div className="flex gap-3">
              ...
              <Button onClick={() => navigate('/')} variant="outline" className="flex-1 border-[#2A2A2A] text-[#888] hover:text-white rounded-lg py-5">Continuar Comprando</Button>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {step === 1 && (
              <div className="dark-card rounded-xl p-6 animate-fadeIn">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div className="sm:col-span-2"><Label className="text-[#CCC]">Nome Completo *</Label><Input value={address.name} onChange={e => handleAddressChange('name', e.target.value)} className="bg-[#111] border-[#2A2A2A] text-white" placeholder="Seu nome completo" /></div>
                  <div><Label className="text-[#CCC]">CPF *</Label><Input value={address.cpf} onChange={e => handleAddressChange('cpf', e.target.value)} className="bg-[#111] border-[#2A2A2A] text-white" placeholder="000.000.000-00" maxLength={14} /></div>
                  <div><Label className="text-[#CCC]">Telefone *</Label><Input value={address.phone} onChange={e => handleAddressChange('phone', e.target.value)} className="bg-[#111] border-[#2A2A2A] text-white" placeholder="(00) 00000-0000" maxLength={15} /></div>
                  <div><Label className="text-[#CCC]">CEP *</Label><Input value={address.zip_code} onChange={e => handleAddressChange('zip_code', e.target.value)} className="bg-[#111] border-[#2A2A2A] text-white" placeholder="00000-000" maxLength={9} /></div>
                  <div><Label className="text-[#CCC]">Estado *</Label>
                    <select value={address.state} onChange={e => handleAddressChange('state', e.target.value)} className="w-full h-10 px-3 rounded-md bg-[#111] border border-[#2A2A2A] text-white">
                      <option value="">Selecione</option>{STATES.map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                  </div>
                  <div className="sm:col-span-2"><Label className="text-[#CCC]">Rua/Avenida *</Label><Input value={address.street} onChange={e => handleAddressChange('street', e.target.value)} className="bg-[#111] border-[#2A2A2A] text-white" placeholder="Nome da rua" /></div>
                  <div><Label className="text-[#CCC]">Numero *</Label><Input value={address.number} onChange={e => handleAddressChange('number', e.target.value)} className="bg-[#111] border-[#2A2A2A] text-white" placeholder="123" /></div>
                  <div><Label className="text-[#CCC]">Complemento</Label><Input value={address.complement} onChange={e => handleAddressChange('complement', e.target.value)} className="bg-[#111] border-[#2A2A2A] text-white" placeholder="Apto, Bloco..." /></div>
                  <div><Label className="text-[#CCC]">Bairro *</Label><Input value={address.neighborhood} onChange={e => handleAddressChange('neighborhood', e.target.value)} className="bg-[#111] border-[#2A2A2A] text-white" placeholder="Nome do bairro" /></div>
                  <div><Label className="text-[#CCC]">Cidade *</Label><Input value={address.city} onChange={e => handleAddressChange('city', e.target.value)} className="bg-[#111] border-[#2A2A2A] text-white" placeholder="Nome da cidade" /></div>
                </div>
                <Button onClick={() => validateAddress() && setStep(2)} className="w-full gold-btn rounded-lg py-5 mt-6">Continuar para Frete</Button>
              </div>
            )}
                  ...
                  {shippingOptions.map(opt => {
                    ...
                    return (
                      <button key={key} onClick={() => setSelectedShipping(key)}
                        ...
                        <div className="flex-1 text-left"><p className="font-semibold text-white">{opt.name}</p><p className="text-sm text-[#888]">{opt.days}</p></div>
                        <div className="text-right">{opt.price === 0 ? <span className="text-green-400 font-bold">GRATIS</span> : <span className="text-[#B38B36] font-bold">R$ {opt.price.toFixed(2)}</span>}</div>
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${isSelected ? 'border-[#B38B36] bg-[#B38B36]' : 'border-[#555]'}`}>{isSelected && <CheckCircle className="w-3 h-3 text-white" />}</div>
                      </button>
                    );
                  })}
                </div>
                <div className="flex gap-3 mt-6">
                  <Button variant="outline" onClick={() => setStep(1)} className="flex-1 border-[#2A2A2A] text-[#888] hover:text-white">Voltar</Button>
            ...
            {step === 3 && (
              <div className="space-y-6 animate-fadeIn">
                ...
                <div className="dark-card rounded-xl p-6">
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <Input value={couponCode} onChange={e => setCouponCode(e.target.value.toUpperCase())} placeholder="Digite o codigo" className="pl-10 bg-[#111] border-[#2A2A2A] text-white uppercase" disabled={!!appliedCoupon} />
                    </div>
                    {appliedCoupon ? (
                ...
                <div className="dark-card rounded-xl p-6">
                  <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
                    <CreditCard className="w-5 h-5 text-[#B38B36]" /> Como deseja pagar?
                  </h2>

                  {paymentMethods.length === 0 ? (
                    <div className="bg-[#111] rounded-lg p-6 border border-[#2A2A2A] text-center">
                      <AlertCircle className="w-8 h-8 text-[#666] mx-auto mb-2" />
                      <p className="text-[#888]">Nenhum metodo de pagamento configurado pelo administrador.</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {paymentMethods.map(pm => {
                        ...
                        return (
                          <button key={pm.id} onClick={() => setSelectedPayment(pm.id)}
                            className={`w-full p-4 rounded-xl border-2 transition-all flex items-center gap-4 text-left ${isSelected ? `${colors.border} ${colors.activeBg}` : 'border-[#2A2A2A] hover:border-[#444]'}`}>
                            <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${isSelected ? colors.bg : 'bg-[#2A2A2A]'}`}>
                              <span className={isSelected ? colors.text : 'text-[#888]'}>{methodIcons[pm.id] || <CreditCard className="w-5 h-5" />}</span>
                            </div>
                ...
                {selectedMethodData && selectedMethodData.configured && (
                  <div className="dark-card rounded-xl p-6 border border-[#B38B36]/30">
                    <h3 className="font-bold text-[#B38B36] mb-4 flex items-center gap-2">
                      <QrCode className="w-5 h-5" /> Dados para Pagamento
                    </h3>

                    {selectedPayment === 'pix' && selectedMethodData.details?.pix_key && (
                      <div className="space-y-3">
                        <p className="text-[#CCC] text-sm">Apos confirmar, faca um PIX para a chave abaixo:</p>
                        <div className="bg-[#0A0A0A] rounded-lg p-4 border border-[#2A2A2A]">
                          <p className="text-xs text-[#888] mb-1">Chave PIX ({selectedMethodData.details.pix_key_type === 'cpf' ? 'CPF/CNPJ' : selectedMethodData.details.pix_key_type === 'email' ? 'E-mail' : selectedMethodData.details.pix_key_type === 'phone' ? 'Telefone' : 'Aleatoria'})</p>
                          <div className="flex items-center justify-between">
                            <p className="text-white font-mono text-lg break-all">{selectedMethodData.details.pix_key}</p>
                            <button onClick={() => copyToClipboard(selectedMethodData.details.pix_key)} className="text-[#B38B36] hover:text-[#D4A842] p-2 shrink-0"><Copy className="w-5 h-5" /></button>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {selectedPayment === 'ted' && selectedMethodData.details?.bank_name && (
                      <div className="space-y-3">
                        <p className="text-[#CCC] text-sm">Apos confirmar, transfira para a conta abaixo:</p>
                        <div className="bg-[#0A0A0A] rounded-lg p-4 border border-[#2A2A2A] space-y-2">
                          {[['Banco', selectedMethodData.details.bank_name], ['Titular', selectedMethodData.details.account_name], ['Agencia', selectedMethodData.details.bank_branch], ['Conta', selectedMethodData.details.account_number]].map(([label, val]) => (
                            <div key={label} className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-2">
                                <span className="text-white font-medium">{val}</span>
                                {val && <button onClick={() => copyToClipboard(val)} className="text-[#B38B36]"><Copy className="w-3.5 h-3.5" /></button>}
                              </div>
                            </div>
                          ))}
                        </div>
                    ...
                    {selectedPayment === 'paypal' && selectedMethodData.details?.paypal_email && (
                      <div className="space-y-3">
                        <p className="text-[#CCC] text-sm">Apos confirmar, envie o pagamento para:</p>
                        <div className="bg-[#0A0A0A] rounded-lg p-4 border border-[#2A2A2A]">
                          <div className="flex items-center justify-between">
                            <p className="text-white font-medium">{selectedMethodData.details.paypal_email}</p>
                            <button onClick={() => copyToClipboard(selectedMethodData.details.paypal_email)} className="text-[#B38B36] p-2"><Copy className="w-5 h-5" /></button>
                          </div>
                        </div>
                      </div>
                    )}

                    <div className="mt-3 p-3 rounded-lg bg-[#B38B36]/10 border border-[#B38B36]/20">
                      <p className="text-[#B38B36] text-sm">Valor total a pagar: <span className="font-bold text-lg">R$ {total.toFixed(2)}</span></p>
                    </div>
                  </div>
                )}
                ...
                <div className="dark-card rounded-xl p-4">
                  ...
                  <p className="text-[#CCC] text-sm">{address.neighborhood}, {address.city} - {address.state} | CEP: {address.zip_code}</p>
                </div>

                {/* Action Buttons */}
                <div className="flex gap-3">
                  <Button variant="outline" onClick={() => setStep(2)} className="flex-1 border-[#2A2A2A] text-[#888] hover:text-white">Voltar</Button>
          ...
          <div className="lg:col-span-1">
            <div className="dark-card rounded-xl p-6 sticky top-24">
              ...
              <div className="border-t border-[#2A2A2A] pt-4 space-y-2">
                <div className="flex justify-between text-sm"><span className="text-[#888]">Subtotal</span><span className="text-white">R$ {subtotal.toFixed(2)}</span></div>
                <div className="flex justify-between text-sm"><span className="text-[#888]">Frete</span><span className={shippingCost === 0 ? 'text-green-400' : 'text-white'}>{shippingCost === 0 ? 'GRATIS' : `R$ ${shippingCost.toFixed(2)}`}</span></div>
                {discount > 0 && <div className="flex justify-between text-sm"><span className="text-[#888]">Desconto</span><span className="text-green-400">-R$ {discount.toFixed(2)}</span></div>}
                <div className="flex justify-between text-lg font-bold pt-2 border-t border-[#2A2A2A]"><span className="text-white">Total</span><span className="text-[#B38B36]">R$ {total.toFixed(2)}</span></div>
              </div>
            </div>
          </div>
        </div>
