...
export function generateFallback(id) {
  ...
  try {
    ...
    const g = grad(ctx, 0, 0, 0, H, [[0, '#1a1a2e'], [0.3, '#16213e'], [0.7, '#0f3460'], [1, '#1a1a2e']]);
    ctx.fillStyle = g;
    ctx.fillRect(0, 0, W, H);
    ctx.fillStyle = '#e94560';
    ctx.font = 'bold 28px sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText((id || 'brane').toUpperCase().replace(/-/g, ' '), W / 2, H / 2 - 10);
    ctx.fillStyle = 'rgba(255,255,255,0.15)';
    ctx.font = '14px sans-serif';
    ctx.fillText('Clique nos pontos para navegar', W / 2, H / 2 + 30);
