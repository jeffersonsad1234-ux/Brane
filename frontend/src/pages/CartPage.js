          ...
          <>
            <div className="space-y-3 mb-6">
              {items.map(item => {
                ...
                return (
                  <div key={item.item_id} className="bg-transparent border border-[#2A2A2A] rounded-xl p-4 flex items-center gap-4 hover:border-[#B38B36]/30 transition-colors" data-testid={`cart-item-${item.item_id}`}>
                    ...
                    <div className="flex items-center gap-2">
                      <Button variant="outline" size="icon" className="w-8 h-8 border-[#2A2A2A] text-white hover:border-[#B38B36]" onClick={() => updateQty(item.item_id, Math.max(1, item.quantity - 1))} data-testid={`qty-minus-${item.item_id}`}><Minus className="w-3 h-3" /></Button>
                      <span className="text-sm font-medium w-6 text-center text-white">{item.quantity}</span>
                      <Button variant="outline" size="icon" className="w-8 h-8 border-[#2A2A2A] text-white hover:border-[#B38B36]" onClick={() => updateQty(item.item_id, item.quantity + 1)} data-testid={`qty-plus-${item.item_id}`}><Plus className="w-3 h-3" /></Button>
                    </div>
            ...
            <div className="dark-card rounded-xl p-6">
              <div className="flex justify-between items-center mb-4">
                <span className="text-[#888]">Subtotal ({items.length} {items.length === 1 ? 'item' : 'itens'})</span>
              ...
              <Button 
                variant="ghost" 
                className="w-full mt-3 text-[#888] hover:text-white"
