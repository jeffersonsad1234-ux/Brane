...
import { X, User, Mail, Lock, LifeBuoy, KeyRound } from "lucide-react";
import { useAuth } from "../contexts/AuthContext";
import PublicSupportModal from "./PublicSupportModal";
...
export default function BLivreAuthModal({ isOpen, onClose, onAuthSuccess }) {
  ...
  const [supportMode, setSupportMode] = useState("support");
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
      <div className="w-full max-w-md rounded-3xl border border-[#D4A24C]/25 bg-[#0B0B12] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.62)]">
        ...
        {/* Recuperação de conta + Suporte */}
        <div className="mt-4 pt-4 border-t border-white/10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-[12px]">
          <button
            type="button"
            onClick={() => { setSupportMode('recovery'); setSupportOpen(true); }}
            className="inline-flex items-center justify-center gap-1.5 text-[#8C8F9A] hover:text-[#D4A24C] transition-colors py-1"
            data-testid="blivre-forgot-account"
          >
            <KeyRound size={13} />
            Esqueci minha conta
          </button>
          <button
            type="button"
            onClick={() => { setSupportMode('support'); setSupportOpen(true); }}
            className="inline-flex items-center justify-center gap-1.5 text-[#8C8F9A] hover:text-[#D4A24C] transition-colors py-1"
            data-testid="blivre-talk-to-support"
          >
            <LifeBuoy size={13} />
            Falar com suporte
          </button>
