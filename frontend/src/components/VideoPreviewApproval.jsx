...
export default function VideoPreviewApproval({
  ...
  const startPlayback = useCallback(() => {
    ...
    timerRef.current = setInterval(() => {
      ...
      for (let i = 0; i < scenes.length; i++) {
        ...
        if (elapsedMs / 1000 < accum) { setCurrentScene(i); break; }
      }
      if (pct >= 1) { clearInterval(timerRef.current); setPlaying(false); setProgress(1); }
    }, 100);
  }, [duration, scenes]);
  ...
  const handlePlayPause = () => {
    ...
    if (playing) { clearInterval(timerRef.current); setPlaying(false); }
      ...
      if (progress >= 1) { setProgress(0); setElapsed(0); setCurrentScene(0); }
      startPlayback();
    }
  };

  const handleRestart = () => {
    if (hasRealVideo && videoRef.current) { videoRef.current.currentTime = 0; videoRef.current.play(); setPlaying(true); setProgress(0); setElapsed(0); return; }
    clearInterval(timerRef.current);
    setPlaying(false); setProgress(0); setElapsed(0); setCurrentScene(0);
  };
  ...
  const handleVideoTimeUpdate = () => {
    if (videoRef.current && duration > 0) {
      ...
      for (let i = 0; i < scenes.length; i++) {
        accum += scenes[i].duracao;
        if (videoRef.current.currentTime < accum) { setCurrentScene(i); break; }
      }
    ...
    <div className="aa-camp-card">
      ...
      {generating && (
        <div className="vp-generating">
          ...
          <div className="vp-generating-text">{genStatus || 'Gerando...'} {genProgress ? `${Math.round(genProgress * 100)}%` : ''}</div>
          <div className="vp-generating-bar"><div className="vp-generating-fill" style={{ width: `${(genProgress || 0) * 100}%` }} /></div>
        </div>
      )}
        ...
        <div className="vp-player">
          {hasRealVideo ? (
            <div className="vp-screen vp-screen-real">
              <video ref={videoRef} src={realVideoUrl} className="vp-real-video" onTimeUpdate={handleVideoTimeUpdate} onEnded={() => setPlaying(false)} onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} playsInline preload="auto" />
              {!playing && (
            ...
            <div className="vp-screen" onClick={handlePlayPause} style={{ background: scenes[currentScene]?.cor || '#1a1a2e' }}>
              ...
              <div className="vp-persona-badge"><span>{video?.personaNome || video?.pessoa?.nome || 'Apresentador'}</span><span className="vp-persona-style">{video?.pessoa?.estilo || video?.estiloVisual || ''}</span></div>
              ...
              {!playing && progress === 0 && <div className="vp-play-overlay" onClick={handlePlayPause}><span className="vp-play-icon">▶</span><span className="vp-play-text">Assistir prévia</span></div>}
              {!playing && progress > 0 && progress < 1 && <div className="vp-paused-badge">⏸ Pausado</div>}
              {!playing && progress >= 1 && <div className="vp-play-overlay" onClick={handlePlayPause}><span className="vp-play-icon">↻</span><span className="vp-play-text">Repetir</span></div>}
            </div>
          )}
          ...
          <div className="vp-controls">
            <div className="vp-progress-bar">
              ...
              {!hasRealVideo && scenes.map((s, i) => {
                ...
                return <div key={s.id} className={`vp-scene-marker ${i === currentScene ? 'active' : ''}`} style={{ left: `${startPct}%`, width: `${s.duracao / duration * 100}%` }} title={s.tipo} />;
              })}
            </div>
          ...
          <div className="vp-scenes-list">
            ...
            {scenes.map((s, i) => (
              <div key={s.id} className={`vp-scene-item ${i === currentScene ? 'active' : ''}`}>
                <div className="vp-scene-header"><span className="vp-scene-num">{i + 1}</span><span className="vp-scene-tipo">{s.tipo}</span><span className="vp-scene-dur">{formatDuracao(Math.round(s.duracao))}</span></div>
      ...
      <div className="vp-content-section">
        <div className="vp-content-block">
          <div className="vp-content-header">
            ...
            {editMode === 'legenda' ? (
              <div className="vp-edit-actions"><button className="aa-btn aa-btn-sm aa-btn-primary" onClick={handleEditSave}>Salvar</button><button className="aa-btn aa-btn-sm aa-btn-ghost" onClick={handleEditCancel}>Cancelar</button></div>
            ) : (<button className="aa-btn aa-btn-sm aa-btn-ghost" onClick={() => handleEditStart('legenda')}>✏️ Editar</button>)}
          </div>
          {editMode === 'legenda' ? <textarea className="aa-input vp-edit-textarea" value={editText} onChange={e => setEditText(e.target.value)} /> : <div className="vp-content-text">{legenda}</div>}
        </div>
        <div className="vp-content-block">
          <div className="vp-content-header">
          ...
          {editMode === 'roteiro' ? <textarea className="aa-input vp-edit-textarea" value={editText} onChange={e => setEditText(e.target.value)} /> : <div className="vp-content-text">{video?.narracaoCompleta || ''}</div>}
        </div>
      </div>

      <div className="aa-camp-hashtags" style={{ marginTop: 12 }}>{hashtags?.slice(0, 8).map((h, i) => <span key={i} className="aa-camp-hashtag">{h}</span>)}</div>
