...
@api_router.post("/generate-presentation")
async def generate_presentation(req: PresentationRequest):
    ...
    try:
        ...
        subtitle.text = "Generated by BrandPy AI"
        
        # Add content slides
        for i in range(req.slides - 1):
            ...
            tf.text = f"Key point about {req.topic}"
        ...
        return {
            "success": True,
            "presentation": f"data:application/vnd.openxmlformats-officedocument.presentationml.presentation;base64,{pptx_base64}",
            "topic": req.topic,
            "slides": req.slides
        }
...
# AI Browser / Web Search (usando Serper API ou DuckDuckGo)
...
async def web_search(req: SearchRequest):
    ...
    """
    try:
        # Try DuckDuckGo (100% free, no API key needed)
        search_url = "https://html.duckduckgo.com/html/"
        params = {"q": req.query}
        
        response = http_requests.post(search_url, data=params, timeout=10)
        ...
        if response.status_code == 200:
            ...
            for result in soup.find_all('div', class_='result')[:req.num_results]:
