...
class PostCreate(BaseModel):
    content: str = ""
    media: List[str] = []  # URLs/paths to images/videos
    media_type: Optional[str] = "image"  # image, video, mixed
...
class GroupCreate(BaseModel):
    ...
    description: Optional[str] = ""
    cover: Optional[str] = ""
    privacy: Optional[str] = "public"  # public, private
...
class SocialAdCreate(BaseModel):
    title: str
    body: Optional[str] = ""
    image: str
    link: str
    cta: Optional[str] = "Saiba mais"
    active: Optional[bool] = True

class SocialAdUpdate(BaseModel):
    title: Optional[str] = None
...
# ==================== SOCIAL HELPERS ====================
def clean_user_social(user: dict) -> dict:
    """Return public social fields only"""
    return {
...
async def enrich_post(post: dict, viewer_id: Optional[str] = None) -> dict:
    """Attach author info + viewer-specific flags"""
    author = await db.users.find_one({"user_id": post.get("author_id")}, {"_id": 0, "password_hash": 0})
    post["author"] = clean_user_social(author) if author else None
    post["liked"] = False
    if viewer_id:
        like = await db.post_likes.find_one({"post_id": post["post_id"], "user_id": viewer_id})
        post["liked"] = bool(like)
    return post

# ==================== SOCIAL: POSTS ====================
@api_router.post("/social/posts")
async def social_create_post(data: PostCreate, request: Request):
    user = await get_current_user(request)
    if not data.content.strip() and not data.media:
    ...
    return await enrich_post(clean, user["user_id"])

@api_router.get("/social/feed")
async def social_feed(request: Request, limit: int = 20, skip: int = 0):
    # Try auth but allow anonymous
    viewer_id = None
    try:
        ...
        pass
    posts = await db.posts.find({"is_ad": {"$ne": True}}, {"_id": 0}).sort("created_at", -1).skip(skip).limit(limit).to_list(limit)
    # Get active social ads
    ads = await db.social_ads.find({"active": True}, {"_id": 0}).sort("created_at", -1).to_list(20)

    # Fetch ad settings
    settings = await db.platform_settings.find_one({"key": "social_settings"}, {"_id": 0})
    ad_every = (settings or {}).get("value", {}).get("ad_every", 5) if settings else 5

    enriched = []
    for p in posts:
        enriched.append(await enrich_post(p, viewer_id))

    # Insert ads every N posts
    result = []
    ad_idx = 0
    for i, post in enumerate(enriched):
        result.append(post)
        if ads and (i + 1) % ad_every == 0 and ad_idx < len(ads):
            ad = dict(ads[ad_idx])
            ad["is_ad"] = True
            ad["post_id"] = ad.get("ad_id", f"ad_{ad_idx}")
            result.append(ad)
            ad_idx += 1

    return {"posts": result, "has_more": len(posts) == limit}

@api_router.get("/social/posts/{post_id}")
async def social_get_post(post_id: str, request: Request):
    viewer_id = None
    try:
        u = await get_current_user(request)
        viewer_id = u["user_id"]
    ...
    return await enrich_post(post, viewer_id)

@api_router.delete("/social/posts/{post_id}")
async def social_delete_post(post_id: str, request: Request):
    user = await get_current_user(request)
    post = await db.posts.find_one({"post_id": post_id}, {"_id": 0})
    if not post:
        raise HTTPException(status_code=404, detail="Post nao encontrado")
...
async def social_like_post(post_id: str, request: Request):
    user = await get_current_user(request)
    ...
    if post and post["author_id"] != user["user_id"]:
        await db.notifications.insert_one({
            "notification_id": f"notif_{uuid.uuid4().hex[:12]}", "user_id": post["author_id"],
            "type": "social_like", "message": f"{user['name']} curtiu seu post",
            "link": f"/social/posts/{post_id}",
            "read": False, "created_at": datetime.now(timezone.utc).isoformat()
        })
    return {"liked": True}

@api_router.post("/social/posts/{post_id}/comments")
async def social_comment(post_id: str, data: CommentCreate, request: Request):
    user = await get_current_user(request)
    ...
    if post and post["author_id"] != user["user_id"]:
        await db.notifications.insert_one({
            "notification_id": f"notif_{uuid.uuid4().hex[:12]}", "user_id": post["author_id"],
            "type": "social_comment", "message": f"{user['name']} comentou no seu post",
            "link": f"/social/posts/{post_id}",
            "read": False, "created_at": datetime.now(timezone.utc).isoformat()
        })
...
async def social_get_comments(post_id: str):
...
async def social_share_post(post_id: str, request: Request):
    await get_current_user(request)
    await db.posts.update_one({"post_id": post_id}, {"$inc": {"shares_count": 1}})
    return {"message": "Compartilhado"}

# ==================== SOCIAL: STORIES ====================
@api_router.post("/social/stories")
async def social_create_story(data: StoryCreate, request: Request):
    user = await get_current_user(request)
    if not data.media:
        raise HTTPException(status_code=400, detail="Midia obrigatoria")
...
async def social_list_stories(request: Request):
    # Filter expired
    now = datetime.now(timezone.utc).isoformat()
    stories = await db.stories.find({"expires_at": {"$gt": now}}, {"_id": 0}).sort("created_at", -1).to_list(200)
    # Group by author
    grouped = {}
    for s in stories:
        a = s["author_id"]
        if a not in grouped:
            grouped[a] = {
                "author_id": a, "author_name": s["author_name"],
                "author_picture": s["author_picture"], "stories": []
            }
        grouped[a]["stories"].append(s)
    return {"groups": list(grouped.values())}

@api_router.post("/social/stories/{story_id}/view")
async def social_view_story(story_id: str, request: Request):
    await get_current_user(request)
...
async def social_follow(user_id: str, request: Request):
    me = await get_current_user(request)
    if me["user_id"] == user_id:
        raise HTTPException(status_code=400, detail="Nao pode seguir a si mesmo")
    existing = await db.follows.find_one({"follower_id": me["user_id"], "following_id": user_id})
    if existing:
        await db.follows.delete_one({"follower_id": me["user_id"], "following_id": user_id})
    ...
    await db.notifications.insert_one({
        "notification_id": f"notif_{uuid.uuid4().hex[:12]}", "user_id": user_id,
        "type": "social_follow", "message": f"{me['name']} comecou a seguir voce",
        "link": f"/social/profile/{me['user_id']}",
        "read": False, "created_at": datetime.now(timezone.utc).isoformat()
    })
    return {"following": True}

@api_router.get("/social/followers/{user_id}")
async def social_get_followers(user_id: str):
    fol = await db.follows.find({"following_id": user_id}, {"_id": 0}).to_list(500)
    users = []
    for f in fol:
        u = await db.users.find_one({"user_id": f["follower_id"]}, {"_id": 0, "password_hash": 0})
        if u: users.append(clean_user_social(u))
    return {"followers": users, "count": len(users)}

@api_router.get("/social/following/{user_id}")
async def social_get_following(user_id: str):
    fol = await db.follows.find({"follower_id": user_id}, {"_id": 0}).to_list(500)
    users = []
    for f in fol:
...
async def social_get_profile(user_id: str, request: Request):
    user = await db.users.find_one({"user_id": user_id}, {"_id": 0, "password_hash": 0})
    if not user:
        raise HTTPException(status_code=404, detail="Usuario nao encontrado")
    followers_count = await db.follows.count_documents({"following_id": user_id})
    following_count = await db.follows.count_documents({"follower_id": user_id})
    posts = await db.posts.find({"author_id": user_id, "is_ad": {"$ne": True}}, {"_id": 0}).sort("created_at", -1).limit(30).to_list(30)
    posts_count = await db.posts.count_documents({"author_id": user_id, "is_ad": {"$ne": True}})
    is_following = False
    try:
        me = await get_current_user(request)
        f = await db.follows.find_one({"follower_id": me["user_id"], "following_id": user_id})
        is_following = bool(f)
    except HTTPException:
        pass
    profile = clean_user_social(user)
    return {
        **profile,
...
async def social_update_profile(data: SocialProfileUpdate, request: Request):
    user = await get_current_user(request)
    updates = {k: v for k, v in data.model_dump(exclude_none=True).items()}
    if updates:
        await db.users.update_one({"user_id": user["user_id"]}, {"$set": updates})
    updated = await db.users.find_one({"user_id": user["user_id"]}, {"_id": 0, "password_hash": 0})
    return clean_user_social(updated)

@api_router.get("/social/suggestions")
async def social_suggestions(request: Request):
    me = await get_current_user(request)
    following = await db.follows.find({"follower_id": me["user_id"]}, {"_id": 0}).to_list(500)
    following_ids = {f["following_id"] for f in following}
    following_ids.add(me["user_id"])
    users = await db.users.find(
        {"user_id": {"$nin": list(following_ids)}},
        {"_id": 0, "password_hash": 0}
    ).limit(10).to_list(10)
    return {"suggestions": [clean_user_social(u) for u in users]}

# ==================== SOCIAL: MESSAGES ====================
@api_router.post("/social/messages")
async def social_send_message(data: MessageSend, request: Request):
    me = await get_current_user(request)
    if not data.content.strip():
        raise HTTPException(status_code=400, detail="Mensagem vazia")
    thread_id = "_".join(sorted([me["user_id"], data.to_user_id]))
    msg = {
    ...
    await db.notifications.insert_one({
        "notification_id": f"notif_{uuid.uuid4().hex[:12]}", "user_id": data.to_user_id,
        "type": "social_message", "message": f"{me['name']} enviou uma mensagem",
        "link": f"/social/messages/{me['user_id']}",
        "read": False, "created_at": datetime.now(timezone.utc).isoformat()
    })
    return {k: v for k, v in msg.items() if k != "_id"}

@api_router.get("/social/messages/threads")
async def social_list_threads(request: Request):
    me = await get_current_user(request)
    # Get all messages involving me
    msgs = await db.direct_messages.find(
        {"$or": [{"from_user_id": me["user_id"]}, {"to_user_id": me["user_id"]}]},
        {"_id": 0}
    ).sort("created_at", -1).to_list(1000)
    threads = {}
    for m in msgs:
        tid = m["thread_id"]
        other_id = m["to_user_id"] if m["from_user_id"] == me["user_id"] else m["from_user_id"]
        if tid not in threads:
            u = await db.users.find_one({"user_id": other_id}, {"_id": 0, "password_hash": 0})
            threads[tid] = {
                "thread_id": tid,
                "other_user": clean_user_social(u) if u else {"user_id": other_id, "name": "Usuario"},
                "last_message": m,
                "unread_count": 0,
            }
        if m["to_user_id"] == me["user_id"] and not m.get("read"):
            threads[tid]["unread_count"] += 1
    return {"threads": list(threads.values())}

@api_router.get("/social/messages/{other_user_id}")
async def social_get_thread(other_user_id: str, request: Request):
    me = await get_current_user(request)
    thread_id = "_".join(sorted([me["user_id"], other_user_id]))
    msgs = await db.direct_messages.find({"thread_id": thread_id}, {"_id": 0}).sort("created_at", 1).to_list(500)
    # Mark as read
    await db.direct_messages.update_many(
        {"thread_id": thread_id, "to_user_id": me["user_id"], "read": False},
        {"$set": {"read": True}}
    )
    other = await db.users.find_one({"user_id": other_user_id}, {"_id": 0, "password_hash": 0})
    return {
        "messages": msgs,
        "other_user": clean_user_social(other) if other else None,
    }

# ==================== SOCIAL: GROUPS ====================
@api_router.post("/social/groups")
async def social_create_group(data: GroupCreate, request: Request):
    user = await get_current_user(request)
    group_id = f"grp_{uuid.uuid4().hex[:12]}"
    group = {
        "group_id": group_id, "name": data.name,
        "description": data.description or "",
        "cover": data.cover or "",
...
async def social_list_groups():
    groups = await db.groups.find({}, {"_id": 0}).sort("members_count", -1).to_list(100)
    return {"groups": groups}

@api_router.get("/social/groups/{group_id}")
async def social_get_group(group_id: str, request: Request):
    group = await db.groups.find_one({"group_id": group_id}, {"_id": 0})
    if not group:
        raise HTTPException(status_code=404, detail="Grupo nao encontrado")
    posts = await db.group_posts.find({"group_id": group_id}, {"_id": 0}).sort("created_at", -1).limit(50).to_list(50)
    for p in posts:
        author = await db.users.find_one({"user_id": p["author_id"]}, {"_id": 0, "password_hash": 0})
        p["author"] = clean_user_social(author) if author else None
    is_member = False
    try:
        me = await get_current_user(request)
        is_member = me["user_id"] in (group.get("members") or [])
    except HTTPException:
        pass
    return {**group, "posts": posts, "is_member": is_member}

@api_router.post("/social/groups/{group_id}/join")
async def social_join_group(group_id: str, request: Request):
    user = await get_current_user(request)
    group = await db.groups.find_one({"group_id": group_id}, {"_id": 0})
    if not group:
        raise HTTPException(status_code=404, detail="Grupo nao encontrado")
    ...
    if user["user_id"] in members:
        await db.groups.update_one(
            {"group_id": group_id},
            {"$pull": {"members": user["user_id"]}, "$inc": {"members_count": -1}}
        )
        return {"joined": False}
    await db.groups.update_one(
        {"group_id": group_id},
        {"$addToSet": {"members": user["user_id"]}, "$inc": {"members_count": 1}}
    )
    return {"joined": True}

@api_router.post("/social/groups/posts")
async def social_create_group_post(data: GroupPostCreate, request: Request):
    user = await get_current_user(request)
    group = await db.groups.find_one({"group_id": data.group_id}, {"_id": 0})
    if not group:
        raise HTTPException(status_code=404, detail="Grupo nao encontrado")
    if user["user_id"] not in (group.get("members") or []):
        raise HTTPException(status_code=403, detail="Voce nao e membro deste grupo")
    post_id = f"gpost_{uuid.uuid4().hex[:12]}"
    post = {
        "post_id": post_id, "group_id": data.group_id,
        "author_id": user["user_id"], "content": data.content,
        "media": data.media, "likes_count": 0, "comments_count": 0,
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
...
async def social_notifications(request: Request):
    user = await get_current_user(request)
    # Social-type notifications
    notifs = await db.notifications.find(
        {"user_id": user["user_id"], "type": {"$regex": "^social_"}},
        {"_id": 0}
...
# ==================== SOCIAL ADS (ADMIN) ====================
@api_router.get("/social/ads")
async def social_list_ads():
    ads = await db.social_ads.find({"active": True}, {"_id": 0}).sort("created_at", -1).to_list(50)
    return {"ads": ads}

@api_router.get("/admin/social/ads")
async def admin_social_list_ads(request: Request):
    await require_admin(request)
    ads = await db.social_ads.find({}, {"_id": 0}).sort("created_at", -1).to_list(100)
    return {"ads": ads}

@api_router.post("/admin/social/ads")
async def admin_social_create_ad(data: SocialAdCreate, request: Request):
    await require_admin(request)
    ad_id = f"sad_{uuid.uuid4().hex[:12]}"
    ad = {
        "ad_id": ad_id, "post_id": ad_id,
        "title": data.title, "body": data.body or "",
        "image": data.image, "link": data.link,
        "cta": data.cta or "Saiba mais",
        "active": data.active if data.active is not None else True,
        "views": 0, "clicks": 0,
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
    await db.social_ads.insert_one(ad)
    return {k: v for k, v in ad.items() if k != "_id"}

@api_router.put("/admin/social/ads/{ad_id}")
async def admin_social_update_ad(ad_id: str, data: SocialAdUpdate, request: Request):
    await require_admin(request)
    updates = {k: v for k, v in data.model_dump(exclude_none=True).items()}
    if updates:
        await db.social_ads.update_one({"ad_id": ad_id}, {"$set": updates})
    return await db.social_ads.find_one({"ad_id": ad_id}, {"_id": 0})

@api_router.delete("/admin/social/ads/{ad_id}")
async def admin_social_delete_ad(ad_id: str, request: Request):
    await require_admin(request)
    await db.social_ads.delete_one({"ad_id": ad_id})
    return {"message": "Anuncio removido"}

@api_router.post("/social/ads/{ad_id}/click")
async def social_ad_click(ad_id: str):
    await db.social_ads.update_one({"ad_id": ad_id}, {"$inc": {"clicks": 1}})
    return {"ok": True}

# ==================== GLOBAL CUSTOMIZATION (EXPANDED) ====================
@api_router.get("/customization")
async def get_customization():
    """Public endpoint - returns all global customization settings"""
    s = await db.platform_settings.find_one({"key": "customization"}, {"_id": 0})
    defaults = {
        # Social colors
        ...
        "contrast_level": "normal",  # low, normal, high
        # Marketplace layout
        "card_size": "medium",  # small, medium, large, custom
        "card_size_custom": 240,  # pixels
        "products_per_row": 4,  # 2, 3, 4
        # Profile layout
        "profile_layout": "modern",  # modern, classic, minimal
        "profile_accent": "#A855F7",
        # Ads
        "social_ad_every": 5,  # show ad every N posts
        "social_ads_enabled": True,
    }
    return {**defaults, **(s.get("value") if s else {})}

@api_router.put("/admin/customization")
async def update_customization(request: Request):
    await require_admin(request)
    body = await request.json()
    existing = await db.platform_settings.find_one({"key": "customization"}, {"_id": 0})
    current = existing.get("value", {}) if existing else {}
    current.update(body)
    await db.platform_settings.update_one(
        {"key": "customization"},
        {"$set": {"key": "customization", "value": current}},
        upsert=True
    )
    return {"message": "Customizacao atualizada", "value": current}

@api_router.get("/admin/customization")
async def admin_get_customization(request: Request):
    await require_admin(request)
    return await get_customization()
