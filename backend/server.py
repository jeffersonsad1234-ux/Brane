...
async def create_social_post(data: SocialPostCreate, request: Request):
    ...
    post = {
        ...
        "enhanced_description": data.enhanced_description or "",
...
async def seed_blivre_posts():
    ...
    seed_data = [
        {"title": "✨ Bem-vindo à B Livre ✨",  "price": "180", "condition": "Usado",    "desc": "Bicicleta aro 28, marchas, em bom estado. Ideal para lazer ou transporte diário.", "img": "https://images.unsplash.com/photo-1532298229144-0ec0c57515c7?w=400&h=400&fit=crop&auto=format"},
        {"title": "🔥 Bem-vindo à B Livre 🔥",  "price": "60",  "condition": "Seminovo", "desc": "Ventilador de parede 40cm, 3 velocidades, controle remoto. Funciona perfeitamente.", "img": "https://images.unsplash.com/photo-1565151443833-29bf2ba5dd8d?w=400&h=400&fit=crop&auto=format"},
        {"title": "💛 Bem-vindo à B Livre 💛",  "price": "90",  "condition": "Usado",    "desc": "Mesa retangular 80x50cm, madeira maciça. Leve desgaste na superfície, ótimo custo.", "img": "https://images.unsplash.com/photo-1737534884876-426964ba462a?w=400&h=400&fit=crop&auto=format"},
        {"title": "🚀 Bem-vindo à B Livre 🚀",  "price": "250", "condition": "Usado",    "desc": "Smartphone Android 64GB, tela 6.1, câmera 12MP. Acompanha carregador e capa.", "img": "https://images.unsplash.com/photo-1758186355698-bd0183fc75ed?w=400&h=400&fit=crop&auto=format"},
        {"title": "🎉 Bem-vindo à B Livre 🎉",  "price": "35",  "condition": "Seminovo", "desc": "Mochila 35L, compartimento para notebook, várias cores. Usada por 1 semestre.", "img": "https://images.unsplash.com/photo-1583300418584-8332e32b710e?w=400&h=400&fit=crop&auto=format"},
        {"title": "⭐ Bem-vindo à B Livre ⭐",  "price": "70",  "condition": "Usado",    "desc": "Tênis casual tamanho 40/41, pouco uso. Confortável para o dia a dia.", "img": "https://images.unsplash.com/photo-1680204101489-2c1319c872b2?w=400&h=400&fit=crop&auto=format"},
        {"title": "💫 Bem-vindo à B Livre 💫",  "price": "120", "condition": "Usado",   "desc": "Cadeira ergonômica com braços ajustáveis, encosto reclinável. Bastante confortável.", "img": "https://images.unsplash.com/photo-1611001476049-a59a2736d410?w=400&h=400&fit=crop&auto=format"},
        {"title": "🌟 Bem-vindo à B Livre 🌟",  "price": "50",  "condition": "Seminovo", "desc": "Liquidificador 2L, 5 velocidades, copo de vidro. Pouquíssimo uso.", "img": "https://images.unsplash.com/photo-1725419281114-9e9fddbdec6d?w=400&h=400&fit=crop&auto=format"},
        {"title": "👋 Bem-vindo à B Livre 👋",  "price": "80",  "condition": "Novo",     "desc": "Capacete fechado, tamanho M, viseira fumê. Certificado INMETRO. Nunca usado.", "img": "https://images.unsplash.com/photo-1718941144539-1d90956a117d?w=400&h=400&fit=crop&auto=format"},
        {"title": "💎 Bem-vindo à B Livre 💎",  "price": "200", "condition": "Usado",    "desc": "Violão aço EAG, som encorpado. Cordas novas, leves marcas de uso no braço.", "img": "https://images.unsplash.com/photo-1505859208100-4a17362f784b?w=400&h=400&fit=crop&auto=format"},
    ]
    posts = []
...
# ==================== TTS ENDPOINT ====================
import tempfile, os, subprocess, json, shutil, time
from pydantic import BaseModel
...
TEMP_TTS_DIR = Path(tempfile.gettempdir()) / "brane-tts"
...
@api_router.post("/tts", response_class=FileResponse)
async def generate_tts(req: TTSRequest):
    ...
    for attempt, v in enumerate(voices_to_try):
        uid = f"tts_{uuid.uuid4().hex[:10]}"
        mp3_path = TEMP_TTS_DIR / f"{uid}.mp3"
        wav_path = TEMP_TTS_DIR / f"{uid}.wav"

        try:
            logger.info(f"[TTS] Attempt {attempt+1}/{len(voices_to_try)}: {v}")
            tts = edge_tts.Communicate(text, voice=v, rate=req.rate, pitch=req.pitch)
            ...
            # FFmpeg: normalize, convert to proper WAV
            try:
                subprocess.run([
                    "ffmpeg", "-y",
                    "-i", str(mp3_path),
                ...
                logger.warning(f"[TTS] FFmpeg failed, using raw MP3: {ff_err}")
                final_path = mp3_path
            ...
            logger.info(f"[TTS] OK: {v} | {size_kb}KB | {duration_s}s | {media_type}")
            ...
            response = FileResponse(
                ...
                headers={
                    ...
                    "X-TTS-Codec": "pcm_s16le" if final_path.suffix == ".wav" else "mp3",
            ...
            last_error = str(e)
            logger.error(f"[TTS] Voice {v} failed: {e}")
    ...
    raise HTTPException(status_code=502, detail=f"TTS falhou apos {len(voices_to_try)} tentativas: {last_error}")
