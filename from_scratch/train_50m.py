"""
BRANPY AI -- TREINO OTIMIZADO
50M params, 10K exemplos selecionados, rapido no CPU
"""
import os, sys, time, json, math, re, random
...
WEIGHTS = BASE / "weights" / "branpy_50m"
...
class BranPy50M(nn.Module):
    def __init__(self, vocab, d_model=512, n_layers=10, n_heads=8, d_ff=2048, max_len=192):
        super().__init__()
        ...
        log(f"Modelo: {n/1e6:.1f}M parametros")
...
class ChatDataset(Dataset):
    def __init__(self, corpus_file, tokenizer, max_len=192, max_examples=10000):
        self.examples = []
        ...
        for pair in pairs:
            ...
            if len(lines) >= 2:
                ...
                if len(ids) > 10:
                    valid.append(torch.tensor(ids, dtype=torch.long))
...
def train():
    log("=" * 60)
    log("BRANPY AI -- TREINO 50M OTIMIZADO")
    log("=" * 60)

    tok = SimpleTokenizer()
    tok_path = str(BASE / "tokenizer_50m.json")
    ...
    ds = ChatDataset(corpus, tok, max_len=192, max_examples=10000)
    dl = DataLoader(ds, batch_size=4, shuffle=True, num_workers=0)
    log(f"Batches: {len(dl)}")

    model = BranPy50M(vocab=len(tok.char_to_id), d_model=512, n_layers=10, n_heads=8, d_ff=2048, max_len=192)
    ...
    log(f"Tempo estimado: ~{len(dl)*0.5*epochs/3600:.1f} horas")
    log("=" * 60)
