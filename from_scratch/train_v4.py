"""
BRANPY AI — TREINO OTIMIZADO V4
- 16.9M params (sabemos que treina em 2.5h/epoch no CPU)
- Tokenizer BPE 8K (melhor que 16K)
- max_len=128 (mais rapido)
- batch_size=8 (mais rapido)
- 30 epochs
"""
...
WEIGHTS = BASE / "weights" / "branpy_v4"
WEIGHTS.mkdir(parents=True, exist_ok=True)
DATA_DIR = BASE / "data"
TOK_PATH = BASE / "tokenizer_v4.json"
...
class BranPy(nn.Module):
    def __init__(self, vocab, d_model=384, n_layers=6, n_heads=6, d_ff=1536, max_len=128, dropout=0.1):
        super().__init__()
        ...
        print(f"Modelo: {n/1e6:.1f}M params | d={d_model} layers={n_layers}")
...
@torch.no_grad()
def generate(model, tok, prompt, max_new=150, temperature=0.8, top_k=50, top_p=0.9):
    model.eval()
...
def train():
    print("=" * 60)
    print("BRANPY AI — TREINO V4 (16.9M, BPE 8K, Otimizado)")
    print("=" * 60)
    ...
    model = BranPy(vocab=vocab_size, d_model=384, n_layers=6, n_heads=6, d_ff=1536, max_len=128)
